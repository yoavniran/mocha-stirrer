...
function mockRequire(cup, requirePath, setupFn, options) {
    ...
    if (!cup._require) {
        throw new ReferenceError("to use this you must provide the require method to the grind");
    }

    var orgRequire = Module.prototype.require;
    var requirePath = cup._require.resolve(requirePath);

    Module.prototype.require = function (path) {

        console.log(" $$$$$$$$$$$$$$$$$ ---- REQUIRE !!!!!!! - " + requirePath);

        var resolvedPath, module, stub;

        if (path === requirePath) {

            console.log(" $$$$$$$$$$$$$$$$$ ---- REAL !!!!!!");

            module = orgRequire.call(this, path);  //do the real require for the module being tested
        }
        else {
            resolvedPath = Module._resolveFilename(path, this);
            stub = Module._cache[resolvedPath];

            if (!stub) {
                console.log(" $$$$$$$$$$$$$$$$$ ---- STUBBING !!!!!!! - " + resolvedPath);

                module = Module._load(resolvedPath, this);

                if (utils.isFunc(module)) {
                    stub = cup.sb.createStubInstance(module);

                    if (utils.isObjectEmpty(stub)) {
                        stub = cup.sb.stub();
                    }
                }
                else {
                    stub = cup.sb.stub(module);
                }

                Module._cache[resolvedPath] = stub;
                module = stub;
            }
        }
    ...
    console.log("checking --- " + (orgRequire === Module.prototype.require));
...
//todo: make the private methods truly private
//todo: add config for stirring pars before or beforeEach
...
var body = (function () {
    ...
    function _stir(conf, each) {
        //console.log("!!!!!!! DOING THE STIRRING", conf);
