...
var blender = (function () {
    ...
    var EMPTY = "-_empty_-",
        _requireParent = module.parent.parent; //by default use the module that required
    ...
    Cup.prototype.getRequired = function (name) {
        //todo: done - allow retrieving the mock-required module using the alias provided
    ...
    function blend(conf) {
        ...
        debug("blending a new cup");
        conf = conf || {};
        conf = utils.clone(conf);      //todo: done - clone the conf object
    ...
    function restir(cup) {
        ...
        //todo: done - move restir stuff belonging to CupStirrer to it
    ...
    function fakeRequire(parent, cup, requirePath, options) {
        //todo: done - allow overriding the parent using options
    ...
    function _createRequire(cup, reqInfo) {
        ...
        if (reqInfo) {
            ...
            if (options.alias) {//todo: done - use alias if one provided
    ...
    function _enrichMocksFromMocker(cup) {
        _takeMocksFromMocker(cup, "stubs", cup._mocker.getStubs());
        _takeMocksFromMocker(cup, "spies", cup._mocker.getSpies());
    }

    function _takeMocksFromMocker(cup, mockType, mocks) {
        ...
        Object.keys(mocks).forEach(function (key) {
            if (!cupList[key]) {
                ...
                _addMockAlias(cup, mockType, key);
            }
        });
    }

    function _addMockAlias(cup, mockType, fullPath) {

        var aliases = cup._mockAliases[mockType];

        if (fullPath.indexOf(path.sep) === 0) {  //if path includes separator so is a local module
