...
var utils = (function () {
    ...
    function getLeveledFileName(fileName, lCount) {
		...
		var levelName = path.basename(dir) + "/" + bareName; //path.join(path.basename(dir), bareName);

        levels[0] = levelName;

        for (var i = 1; i < levels.length; i += 1) {
            dir = path.dirname(dir);
			levelName = path.basename(dir) + "/" + levelName; //cant use path.join because it will give different results based on the OS!   //path.join(path.basename(dir), levelName);
			levels[i] = levelName;
        }
